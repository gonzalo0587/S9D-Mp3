...
import { savePlaylist, type FoundTrack, getPlaylistSummaries, } from "./library-storage.js";
...
const AUDIO_EXTENSIONS = new Set<AudioFileExtension>([
  "mp3",
  "wav",
  "flac",
  "aac",
...
function ensureAppConfig(): AppConfig {
...
function getAudioExtension(filePath: string): AudioFileExtension | null {
  const ext = path.extname(filePath).toLowerCase().replace(".", "");

  if (AUDIO_EXTENSIONS.has(ext as AudioFileExtension)) {
    return ext as AudioFileExtension;
  }

  return null;
}

function scanFolderForAudioFiles(folderPath: string): ScanFolderResult {
...
app.whenReady().then(() => {
  ...
  ipcMain.handle("config:set-library-paths", async (_event, paths: string[]) => {
  ...
  ipcMain.handle("config:add-library-path", async (_event, dirPath: string) => {
    ...
    ipcMain.handle(
    "playlist:save",
    async (_event, payload: { playlistName: string; tracks: FoundTrack[] }) => {
